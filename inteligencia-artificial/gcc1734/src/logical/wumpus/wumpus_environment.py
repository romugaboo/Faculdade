...
class Explorer(agents.Agent):
    ...
    def reset(self):
        ...
        if hasattr(self, 'initial_location'):
            ...
            print("{0}.reset(): agent has no initial_location;".format(
    ...
    def heading_str(self, heading):
        ...
        if isinstance(heading, int):
            if 0 <= heading <= 3:
                ...
                print(
                    "Not a valid heading int (0 <= heading <= 3), got: {0}".format(heading))
                sys.exit(0)
            ...
            if heading in headings:
                ...
                print("Not a valid heading str (one of {0}), got: {1}".format(
                    headings, heading))
                sys.exit(0)
        else:
    ...
    def pretty_percept_vector(self, pvec):
        ...
        if pvec[0]:
            percept_vector.append('Stench')
        else:
            percept_vector.append('~Stench')
        if pvec[1]:
            percept_vector.append('Breeze')
        else:
            percept_vector.append('~Breeze')
        if pvec[2]:
            percept_vector.append('Glitter')
        else:
            percept_vector.append('~Glitter')
        if pvec[3]:
            percept_vector.append('Bump')
        else:
            percept_vector.append('~Bump')
        if pvec[4]:
            percept_vector.append('Scream')
        else:
            percept_vector.append('~Scream')
...
class WumpusEnvironment(agents.XYEnvironment):
        ...
        if action == 'TurnRight':
            ...
            if self.some_things_at(agent.location, tclass=Gold):
                try:
                    ...
                    print('Gold?:', self.list_things_at(
                        agent.location, tclass=Gold))
                    sys.exit(-1)
        ...
        while True:
            ...
            if self.some_things_at(aloc, tclass=Wumpus):
                try:
                    ...
                    print('Wumpus?:', aloc, self.list_things_at(
                        aloc, tclass=Wumpus))
                    sys.exit(-1)
    ...
    def to_string(self, t=None, title=None):
        ...
        Uses R&N AIMA room coordinates: (0,0) is bottom-left in ascii display """
        if title:
            print(title)
        ...
        for r in range(self.height, -1, -1):
            for c in range(0, self.width + 1):
                ...
                spacer = ''.join(
                    [' ' for i in range(column_width - cell_width)])
                slist.append('| ')
