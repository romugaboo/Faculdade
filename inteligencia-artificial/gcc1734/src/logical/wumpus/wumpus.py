...
def with_manual_program(agent):
    ...
    def show_commands():
        ...
        print("     {0}".format(
            ', '.join(['\'{0}\''.format(a) for a in actions])))
    ...
    def manual_program(percept):
        print("[{0}] You perceive: {1}".format(
            agent.time, agent.pretty_percept_vector(percept)))
        action = None
        while not action:
...
def with_manual_kb_program(agent):
    ...
    def show_propositions():
        ...
        print(
            "     '" + '\', \''.join(proposition_bases_state_fluents[:4]) + '\',')
        print(
            "     '" + '\', \''.join(proposition_bases_state_fluents[4:]) + '\'')
    ...
    def check_kb_status():
        ...
        if result:
            ...
            print(
                "Agent KB is NOT satisfiable!!  There is contradiction that needs fixing!")

    def simple_query(proposition):
        """
        Executes a simple query to the agent KB for specified proposition.
        """
        result = agent.kb.ask(logic.expr(proposition))
        if result == None:
    ...
    def location_based_query(proposition_base):
        ...
        for x in range(1, agent.width+1):
            for y in range(1, agent.height+1):
                query = logic.expr('{0}{1}_{2}'.format(proposition_base, x, y))
    ...
    def location_time_based_query(proposition_base, time_step):
        """
        Executes queries for the specified type of proposition, for
        ...
        display_env = WumpusEnvironment(agent.width, agent.height)
        start_time = time.perf_counter()
        print("Running queries for: {0}<x>_<y>_{1}".format(proposition_base, time_step))
        for x in range(1, agent.width+1):
            for y in range(1, agent.height+1):
                query = logic.expr('{0}{1}_{2}_{3}'.format(proposition_base, x, y, time_step))
                result = agent.kb.ask(query)
                if result == None:
                    display_env.add_thing(Proposition(query, '?'), (x, y))
                else:
    ...
    def run_all_queries(time_step):
        ...
        '''
        if time_step-1 > 0:
            print("Actions from previous time: {0}".format(time_step-1))
            for p in proposition_bases_actions:
                simple_query(p + '{0}'.format(time_step-1))

        print("FINISHED running all queries for time {0}".format(time_step))
    ...
    def manual_kb_program(percept):
        ...
        print("         Number of clauses in KB before: {0}".format(
            clauses_before))
        agent.add_temporal_axioms()
        clauses_after = len(agent.kb.clauses)
        print("         Number of clauses in KB after: {0}".format(
            clauses_after))
        print("         Total clauses added to KB: {0}".format(
...
def run_minisat_test():
    ...
    for query, expected_result in queries:
        ...
        print("  Result:     {0}   (Expected: {1})".format(
...
def get_options(argv):
    ...
    parser = argparse.ArgumentParser(
        ...
        usage="""python wumpus.py [-h] [-k] [-y] [-l LAYOUT] [-t]""",
        epilog="""EXAMPLES:
            (1) python wumpus.py
                - starts simple manual Hunt The Wumpus game
            (2) python wumpus.py -k
    ...
    parser.add_argument(
        ...
        "--kb",
        action="store_true",
        help="Instantiate a queriable knowledge base",
    )
    ...
    parser.add_argument(
        ...
        "--hybrid",
        action="store_true",
        help="Run hybrid wumpus agent (takes precedence over -k option)",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="test_minisat",
        help="Test connection to command-line MiniSat",
    )
    ...
    if args.hybrid and args.kb:
        print("Only one of -k or -y can be used at a time")
        exit(1)
...
if __name__ == '__main__':
    """
    The main function called when wumpus.py is run from the command line:
    > python wumpus.py <options>
    """
