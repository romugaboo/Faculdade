...
def plan_route(current, heading, goals, allowed):
    ...
    if goals and allowed:
        prp = PlanRouteProblem(
            (current[0], current[1], heading), goals, allowed)
...
class PlanRouteProblem(search.Problem):
    ...
    def h(self, node):
        ...
        """
        distanceToGoals = [manhattan_distance_with_heading(
            node.state, goal) for goal in self.goals]
...
class PlanShotProblem(search.Problem):
    ...
    def h(self, node):
        ...
        distanceToShotSpots = [manhattan_distance_with_heading(
            node.state, goal) for goal in shotSpots]
