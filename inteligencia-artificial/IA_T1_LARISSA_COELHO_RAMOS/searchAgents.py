...
class CornersProblem(search.SearchProblem):
    ...
    def getNextState(self, state, action):
        ...
        if (nextx, nexty) in self.corners:
            visited_corners[self.corners.index((nextx, nexty))] = True
        
        return (nextx, nexty), tuple(visited_corners)
...
def cornersHeuristic(state, problem):
    ...
    current_position, visited_corners = state[0], state[1]
    unvisited_corners = [corner for i, corner in enumerate(corners) if not visited_corners[i]]
    if not unvisited_corners:
        return 0

    closest_corner_distance = min(util.manhattanDistance(current_position, corner) for corner in unvisited_corners)
    
    corner_distances = [
        util.manhattanDistance(c1, c2) 
        for c1, c2 in combinations(unvisited_corners, 2)
    ]
    mst_approximation = closest_corner_distance + sum(sorted(corner_distances)[:len(unvisited_corners) - 1])
    return mst_approximation
