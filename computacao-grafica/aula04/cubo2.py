...
def cubo(): 
    ...
    indices = array('H',[
        0, 1, 2, 0, 2, 3, # Face 1: A, B, C | A, C, D
        1, 4, 7, 1, 7, 2,
        4, 5, 6, 4, 6, 7,
        0, 5, 6, 0, 6, 3,
        3, 2, 7, 3, 7, 6,
...
def inicializa():
    global progId, cuboVAO, cubo2VAO
...
def desenha():
    ...
    GL.glBindVertexArray(cuboVAO)
    GL.glUseProgram(progId)
    GL.glUniformMatrix4fv(GL.glGetUniformLocation(progId, "mvp"),1,GL.GL_FALSE,glm.value_ptr(mvp2))
    GL.glDrawElements(GL.GL_TRIANGLES,36,GL.GL_UNSIGNED_SHORT,ctypes.c_void_p(0))
