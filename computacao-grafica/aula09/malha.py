...
void main(void) 
{
    color = texture(textureSlot,textura);
}
"""
def compilaShaders():
    error = None
...
def malha(): 
    ...
    GL.glBufferData(GL.GL_ARRAY_BUFFER, len(aTextura)*aTextura.itemsize, ctypes.c_void_p(aTextura.buffer_info()[0]), GL.GL_STATIC_DRAW)
...
def inicializa():
    ...
    loadTexture("opengl/aula09/merda.jpg")
...
def desenha():
    ...
    camera = glm.lookAt(glm.vec3(0,0,12),glm.vec3(0,1,0),glm.vec3(1,0,0))
